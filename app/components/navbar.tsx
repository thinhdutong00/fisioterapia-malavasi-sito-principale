...
export default function Navbar() {
  ...
  const isPrenota = pathname === "/prenota";
  ...
  return (
    <header className={`fixed top-0 left-0 w-full z-[100] transition-all duration-700 ease-[cubic-bezier(0.23,1,0.32,1)] ${isVisible ? "translate-y-0 opacity-100" : "-translate-y-full opacity-0"} ${isScrolled ? "pt-4 px-4" : "pt-0 px-0"} ${pathname === '/prenota' ? 'hidden md:block' : 'block'}`}>
      <div className={`mx-auto transition-all duration-700 ease-[cubic-bezier(0.23,1,0.32,1)] flex items-center justify-between border ${isScrolled ? "max-w-[96%] bg-white/95 backdrop-blur-2xl border-white/40 shadow-[0_20px_50px_-12px_rgba(2,33,102,0.1)] rounded-2xl px-6 md:px-8 py-3" : "max-w-[98%] bg-transparent border-transparent py-8 px-4 md:px-10 rounded-none"}`}>
        
        {/* LOGO */}
        <Link href="/" className="flex items-center flex-shrink-0">
          <div className={`relative transition-all duration-500 ${isScrolled ? "w-36 h-8 md:w-44 h-9" : "w-52 h-10 md:w-72 h-14"}`}>
            <Image src={logoSrc} alt="Logo Malavasi" fill priority className="object-contain transition-all duration-500" />
          </div>
        </Link>

        {/* DESKTOP NAVIGATION */}
        <nav className={`hidden lg:flex items-center flex-shrink transition-all duration-500 ${isScrolled ? "gap-4 xl:gap-6" : "gap-6 xl:gap-10"}`}>
          {navLinks.map((item) => (
                ...
                <div className="flex items-center gap-1 py-2 cursor-default">
                  ...
                  <div className="absolute top-full left-1/2 -translate-x-1/2 pt-4 opacity-0 invisible group-hover/menu:opacity-100 group-hover/menu:visible transition-all duration-300 transform group-hover/menu:translate-y-0 translate-y-2">
                    <div className="bg-white rounded-2xl shadow-2xl border border-slate-100 min-w-[260px] p-2">
                      {item.sub.map((sub) => (
                        <div key={sub.n} className="relative group/inner">
                          <div className="flex items-center justify-between px-5 py-3 hover:bg-slate-50 transition-colors rounded-xl group/item">
                            <Link href={sub.h} className="text-[10px] font-bold uppercase tracking-widest text-[#022166] group-hover/item:text-[#55B4FF] flex-grow">{sub.n}</Link>
                            {sub.innerSub && <ChevronDown size={12} className="-rotate-90 text-slate-300 group-hover/inner:text-[#55B4FF]" />}
                            
                            {/* TENDINA DESKTOP (Livello 2 - Flyout laterale) */}
                            {sub.innerSub && (
                              <div className="absolute left-full top-0 ml-2 opacity-0 invisible group-hover/inner:opacity-100 group-hover/inner:visible transition-all duration-300 transform translate-x-2 group-hover/inner:translate-x-0">
                                <div className="bg-white rounded-2xl shadow-2xl border border-slate-100 min-w-[220px] p-2">
                                  {sub.innerSub.map((inner) => (
                                    <Link key={inner.n} href={inner.h} className="block px-5 py-3 text-[10px] font-bold uppercase tracking-widest text-[#022166] hover:bg-slate-50 hover:text-[#55B4FF] rounded-xl">{inner.n}</Link>
                                  ))}
                                </div>
                              </div>
                            )}
      ...
      {/* MOBILE MENU */}
      <div className={`fixed inset-0 h-screen w-screen bg-white z-[10000] lg:hidden transition-all duration-500 ease-in-out ${isMobileMenuOpen ? "translate-y-0 opacity-100 visible" : "-translate-y-full opacity-0 invisible"}`}>
        <div className="flex flex-col h-full overflow-y-auto">
          <div className="w-full flex items-center justify-between py-8 px-6 border-b border-slate-50 shrink-0">
            <Link href="/" onClick={() => setIsMobileMenuOpen(false)}>
              <div className="relative w-48 h-10"><Image src="/logo-fisioterapia-malavasi.png" alt="Logo Malavasi" fill className="object-contain" /></div>
            </Link>
          ...
          <div className="flex flex-col px-8 pb-12">
                  ...
                  {item.sub ? (
                    <div className="flex flex-col">
                      <div className="flex items-center justify-between w-full">
                        <Link href={item.h} onClick={() => setIsMobileMenuOpen(false)} className="text-lg font-bold text-[#022166] uppercase py-5 flex-grow">{item.n}</Link>
                        ...
                        {item.sub.map((sub) => (
                          <div key={sub.n} className="flex flex-col">
                            <div className="flex items-center justify-between w-full pr-4">
                              <Link href={sub.h} onClick={() => setIsMobileMenuOpen(false)} className="block py-3 pl-4 text-sm font-semibold text-slate-700 flex-grow">{sub.n}</Link>
                              {sub.innerSub && (
                                <button onClick={() => setOpenInnerSubmenu(openInnerSubmenu === sub.n ? null : sub.n)} className="p-3">
                                  <ChevronDown size={16} className={`transition-transform ${openInnerSubmenu === sub.n ? "rotate-180 text-[#55B4FF]" : "text-slate-400"}`} />
                                </button>
                              )}
                            </div>
                            
                            {/* MOBILE SOTTOMENU LIVELLO 2 (Accodion interno) */}
                            {sub.innerSub && (
                              <div className={`overflow-hidden transition-all duration-300 bg-slate-50 rounded-xl ml-4 ${openInnerSubmenu === sub.n ? "max-h-[400px] mb-2" : "max-h-0"}`}>
                                {sub.innerSub.map((inner) => (
                                  <Link key={inner.n} href={inner.h} onClick={() => setIsMobileMenuOpen(false)} className="block py-3 pl-6 text-xs font-medium text-slate-500 border-l border-slate-200">{inner.n}</Link>
                                ))}
                              </div>
                            )}
                          </div>
                    ...
                    <Link href={item.h} onClick={() => setIsMobileMenuOpen(false)} className="text-lg font-bold text-[#022166] uppercase py-5">{item.n}</Link>
                  )}
                </div>
              ))}
            </nav>
            ...
            <div className="mt-10 space-y-4">
              <a href="tel:+393338225464" className="w-full flex items-center justify-center gap-4 bg-slate-100 text-[#022166] py-5 rounded-2xl font-black uppercase text-xs tracking-widest"><Phone size={20} /> Chiama Studio</a>
              <Link href="/prenota" onClick={() => setIsMobileMenuOpen(false)} className="w-full flex items-center justify-center gap-4 bg-[#022166] text-white py-5 rounded-2xl font-black uppercase text-xs tracking-widest shadow-xl"><CalendarCheck size={20} /> Prenota Ora</Link>
            </div>
          </div>
        </div>
      </div>
